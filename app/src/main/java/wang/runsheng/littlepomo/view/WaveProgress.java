package wang.runsheng.littlepomo.view;
...
import wang.runsheng.littlepomo.BuildConfig;
import wang.runsheng.littlepomo.R;
import wang.runsheng.littlepomo.utils.Constant;
import wang.runsheng.littlepomo.utils.MiscUtil;
...
public class WaveProgress extends View {
    ...
    //浅色波浪方向
    private static final int L2R = 0;// 从左向右
    private static final int R2L = 1;// 从右向左
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        isR2L = typedArray.getInt(R.styleable.WaveProgress_lightWaveDirect, R2L) == R2L;// true
    ...
    private void initWavePoints() {
        ...
        // 波浪方向
        mDarkPoints = getPoint(true, waveWidth);
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        drawCircle(canvas);
        drawLightWave(canvas);// 亮的
        drawDarkWave(canvas);// 暗的
        drawProgress(canvas);
    }
    ...
    private void drawDarkWave(Canvas canvas) {
        ...
        drawWave(canvas, mWavePaint, mDarkPoints, isR2L ? -mDarkWaveOffset : mDarkWaveOffset);
    }
